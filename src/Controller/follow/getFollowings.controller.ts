  ...
  export function GetFollowings(fastify: FastifyInstance) {
    fastify.get(
      ...
      async (req: FastifyRequest, res: FastifyReply) => {
        const { userId, entityType } = req.params as { userId: string, entityType: 'user' | 'shop' };
  
        try {
          const followings = await getFollowings(userId, entityType);
          return res.status(200).send(followings);
        } catch (error) {
          return res.status(500).send({ message: 'An error occurred while retrieving followings.' });
        }
      }
    );
  }
